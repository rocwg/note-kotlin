6Working with collections and sequences
This chapter covers

Working with collections in a functional style Sequences: performing collection operations lazily
...
6.1Functional APIs for collections
...
6.1.1Removing and transforming elements: filter and map
...
fun main() {
val people = listOf(Person("Alice", 29), Person("Bob", 31)) println(people.filter { it.age > 30 })
...
fun main() {
val people = listOf(Person("Alice", 29), Person("Bob", 31)) println(people.map { it.name })
// [Alice, Bob]
}
...
people.filter {
val oldestPerson = people.maxByOrNull(Person::age) it.age == oldestPerson?.age
}
...
val maxAge = people.maxByOrNull(Person::age)?.age people.filter { it.age == maxAge }
...
fun main() {
val numbers = mapOf(0 to "zero", 1 to "one") println(numbers.mapValues { it.value.uppercase() })
// {0=ZERO, 1=ONE}
}
...
6.1.2Accumulating values for collections: reduce and fold
...
fun main() {
val people = listOf( Person("Aleksei", 29),
Person("Natalia", 28)
)
...
6.1.3Applying a predicate to a collection: all, any, none, count, and find
...
false
If you need to check whether there’s at least one matching element, use any: fun main() {
...
fun main() {
val list = listOf(1, 2, 3) println(!list.all { it == 3 }) #1
// true
...
Likewise, you can replace !any with none: fun main() {
val list = listOf(1, 2, 3) println(!list.any { it == 4 }) #1
// true
...
If you want to know how many elements satisfy a predicate, use count: fun main() {
...
6.1.4Splitting a list to a pair of lists: partition
...
val comeIn = people.filter(canBeInClub27) val stayOut = people.filterNot(canBeInClub27) println(comeIn)
// [Person(name=Alice, age=26)] println(stayOut)
...
val (comeIn, stayOut) = people.partition(canBeInClub27) #1 println(comeIn)
// [Person(name=Alice, age=26)] println(stayOut)
// [Person(name=Bob, age=29), Person(name=Carol, age=31)]
...
6.1.5Converting a list to a map of groups: groupBy
...
fun main() {
val list = listOf("apple", "apricot", "banana", "cantaloupe") println(list.groupBy(String::first))
...
fun main() {
val people = listOf( Person("Joe", 22),
Person("Mary", 31),
Person("Jamie", 22)
)
val personToAge = people.associateWith { it.age } println(personToAge)
// {Person(name=Joe, age=22)=22, Person(name=Mary, age=31)=31
// Person(name=Jamie, age=22)=22} #1
val ageToPerson = people.associateBy { it.age } #2 println(ageToPerson)
...
6.1.7Replacing elements in mutable collections: replaceAll and fill
...
fun main() {
val names = mutableListOf("Martin", "Samuel") println(names)
// [Martin, Samuel] names.replaceAll { it.uppercase() } println(names)
// [MARTIN, SAMUEL]
names.fill("(redacted)") println(names)
// [(redacted), (redacted)]
}

6.1.8Handling special cases for collections: ifEmpty
...
val full = listOf("apple", "orange", "banana") println(empty.ifEmpty { listOf("no", "values", "here") })
// [no, values, here]
...
6.1.9Splitting collections: "chunked" and "windowed"
...
val names = listOf("Joe", "Mary", "Jamie") val ages = listOf(22, 31, 22, 44, 0)
...
fun main() {
val names = listOf("Joe", "Mary", "Jamie") val ages = listOf(22, 31, 22, 44, 0) println(names.zip(ages))
// [(Joe, 22), (Mary, 31), (Jamie, 22)] #1 println(names.zip(ages) { name, age -> Person(name, age) })
...
val countries = listOf("DE", "NL", "US") println(names zip ages zip countries)
// [((Joe, 22), DE), ((Mary, 31), NL), ((Jamie, 22), US)] #1

6.1.11Processing elements in nested collections: flatMap and flatten
...
val library = listOf(
Book("Kotlin in Action", listOf("Isakova", "Elizarov", "Aigner", Book("Atomic Kotlin", listOf("Eckel", "Isakova")),
...
fun main() {
val authors = library.map { it.authors } println(authors)
...
fun main() {
val authors = library.flatMap { it.authors } println(authors) #1
// [Isakova, Elizarov, Aigner, Jemerov, Eckel, Isakova, Liu] println(authors.toSet()) #2
...
6.2Lazy collection operations: sequences
...
6.2.1Executing sequence operations: intermediate and terminal operations
...
val numbersTo100 = naturalNumbers.takeWhile { it <= 100 } println(numbersTo100.sum()) #1
// 5050
}
...
generateSequence(this) { it.parentFile }.any { it.isHidde
...
Functions from the associate and groupBy families help you turn flat lists into maps, so you can structure your data by your own critera.
